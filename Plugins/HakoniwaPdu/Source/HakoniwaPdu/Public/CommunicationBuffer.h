...
struct HAKONIWAPDU_API FPduBufferMeta
{
    uint64 RecvCounter = 0;
    double RecvUeTime = 0.0;
    int32 DataSize = 0;
};
...
private:
    ...
    TMap<FString, FPduBufferMeta> PduBufferMeta;
    TSet<FString> DeclaredReadPduChannels;
...
public:
    const FPduChannelConfigStruct& GetChannelConfig() const
    {
    ...
    {
        if (ChannelConfig.robots.Num() > 0)
        {
            return ChannelConfig.robots[0].name;
        }
        return TEXT("");
    }
    ...
    void SetBufferWithMeta(const FString& Key, const TArray<uint8>& Data, const FPduBufferMeta& Meta)
    {
        FScopeLock Lock(&Mutex);
        PduBuffer.Add(Key, Data);
        PduBufferMeta.Add(Key, Meta);
    }
    ...
    TArray<uint8> GetBuffer(const FString& Key)
    {
        ...
        {
            PduBufferMeta.Remove(Key);
            return Data;
        }
        return {};
    }

    bool GetBufferWithMeta(const FString& Key, TArray<uint8>& OutData, FPduBufferMeta& OutMeta)
    {
        FScopeLock Lock(&Mutex);
        if (!PduBuffer.RemoveAndCopyValue(Key, OutData))
        {
            OutMeta = FPduBufferMeta();
            return false;
        }

        if (!PduBufferMeta.RemoveAndCopyValue(Key, OutMeta))
        {
            OutMeta = FPduBufferMeta();
            OutMeta.DataSize = OutData.Num();
        }
        return true;
    }
    ...
    void Clear()
    {
        ...
        PduBuffer.Empty();
        PduBufferMeta.Empty();
        DeclaredReadPduChannels.Empty();
    }

    FString GetChannelKey(const FString& RobotName, int32 ChannelId) const
    {
        return RobotName + Separator + FString::FromInt(ChannelId);
    }

    void MarkPduDeclaredForRead(const FString& RobotName, int32 ChannelId)
    {
        FScopeLock Lock(&Mutex);
        DeclaredReadPduChannels.Add(GetChannelKey(RobotName, ChannelId));
    }

    bool IsPduDeclaredForRead(const FString& RobotName, int32 ChannelId)
    {
        FScopeLock Lock(&Mutex);
        return DeclaredReadPduChannels.Contains(GetChannelKey(RobotName, ChannelId));
    }
    ...
    void PutPacketDirectWithMeta(const FString& RobotName, int32 ChannelId, const TArray<uint8>& PduData, const FPduBufferMeta& Meta)
    {
        FString PduName = GetPduName(RobotName, ChannelId);
        if (PduName.IsEmpty())
        {
        ...
        FString Key = GetKey(RobotName, PduName);
        SetBufferWithMeta(Key, PduData, Meta);
    }
};
